class DevicesController < ApplicationController
  ...
  def add_to_ticket
    ...
    TicketEntry.create(:entry_type => "Added Existing Equipment", :note => "Equipment (Service Tag: #{@device.service_tag}) was added to this ticket.", :billable => false, :private => true, :detail => 6, :ticket => @ticket, :creator_id => current_user.id)
  ...
  def remove_from_ticket
    ...
    TicketEntry.create(:entry_type => "Removed Equipment", :note => "Equipment (Service Tag: #{@device.service_tag}) was removed from this ticket.", :billable => false, :private => true, :detail => 6, :ticket => @ticket, :creator_id => current_user.id)
    redirect_to ticket_path(@ticket)
  end
  ...
  def create
    ...
    respond_to do |format|
      if @device.save
        if params[:ticket_id] != nil
          TicketEntry.create(:entry_type => "Added New Equipment", :note => "New Equipment (Service Tag: #{@device.service_tag}) was added to this ticket.", :billable => false, :private => true, :detail => 6, :ticket => @ticket, :creator_id => current_user.id)

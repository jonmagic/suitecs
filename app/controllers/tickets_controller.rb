    ...
    respond_to do |format|
      if @ticket.save
        if params[:device_id] != nil
          ...
          TicketEntry.create(:entry_type => "Add Equipment", :note => "Equipment (Service Tag: #{@device.service_tag}) was added to this ticket.", :billable => false, :private => true, :detail => 6, :ticket => @ticket, :creator_id => current_user.id)
        end
